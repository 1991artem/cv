function LinkedInfo() {
  return ( 
    <a className="contacts_info" href="https://www.linkedin.com/in/artem-puzik-797668235/" target="_blank" rel="noreferrer" title="artem-puzik-797668235">
    <svg
      enableBackground="new 0 0 100 100"
      id="Layer_3"
      version="1.1"
      viewBox="0 0 100 100"
      xmlSpace="preserve"
    >
      <g>
        <path
          d="M26.912,58.394h11.379v35.058c2.937,0.789,5.99,1.292,9.132,1.47v-24.85c0-4.566-0.15-8.459-0.299-11.678   h9.881l0.524,5.016h0.225c1.497-2.321,5.24-5.839,11.304-5.839c7.486,0,13.101,4.94,13.101,15.72v8.175   C90.099,73.351,95,62.25,95,50C95,25.147,74.853,5,50,5S5,25.147,5,50c0,16.411,8.788,30.765,21.912,38.627V58.394z M32.602,39.449   c3.668,0,5.989,2.47,6.063,5.764c0,3.144-2.396,5.689-6.139,5.689c-3.593,0-5.988-2.545-5.914-5.689   C26.538,41.919,28.934,39.449,32.602,39.449z"
          fill="#0274B3"
        />
        <path
          d="M65.015,66.778c-3.144,0-5.015,2.171-5.764,4.267c-0.299,0.674-0.449,1.797-0.449,2.845v20.244   c4.238-0.841,8.258-2.281,11.978-4.221V74.713C70.779,69.997,69.132,66.778,65.015,66.778z"
          fill="#0274B3"
        />
      </g>
    </svg>
    <p>
      artem-puzik-797668235
    </p>
  </a>
    );
}

export default LinkedInfo;